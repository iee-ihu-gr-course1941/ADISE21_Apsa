<?php
	...
	public function addCard(Mistigriff_Card $card,$location = false){
		if(!$locaton){
			$location = Mistigriff_Deck::$CARD_LOCATION_TOP;
		}
		if($location == Mistigriff_Deck::$CARD_LOCATION_TOP){
			$card->setOwner($this);
			array_unshift($this->_cards,$card);
		}
		else if($location == Mistigriff_Deck::$CARD_LOCATION_BOTTOM){
			$card->setOwner($this);
	...
	public function removeCard(Mistigriff_Card $card){
	...
	public function findCards(Mistigriff_Card $template){
		$returnCards = array();
		foreach($this->_cards as $card){
